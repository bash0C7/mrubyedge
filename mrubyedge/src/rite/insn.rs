...
pub enum OpCode {
    ...
    // mruby 4.0 (RITE0400) additions; numbered after every 3.x opcode so ENUM_TABLE_V3 is unchanged.
    GETIDX0,
    MATCHERR,
    SSEND0,
    SEND0,
...
impl RiteVersion {
    pub fn from_major(major: &[u8; 2]) -> Result<Self, super::Error> {
        match major {
            b"03" => Ok(RiteVersion::V3),
            b"04" => Ok(RiteVersion::V4),
            _ => Err(super::Error::UnsupportedVersion(*major)),
        }
    }
...
// mruby 4.0 numbering. LOADI8/LOADTRUE/LOADFALSE are 3.x's LOADI/LOADT/LOADF renamed, so they
// map onto the same variants and a dump shows the 3.x names.
#[rustfmt::skip]
...
pub type FetchFn = fn(&mut &[u8]) -> Result<Fetched, Error>;
