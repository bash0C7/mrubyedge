...
fn do_return(vm: &mut VM, value: Option<Rc<RObject>>) -> Result<(), Error> {
    ...
    if let Some(environ) = vm.cur_env.get(&env_key) {
...
pub(crate) fn op_eq(vm: &mut VM, operand: &Fetched) -> Result<(), Error> {
    ...
    let recv = vm.get_current_regs_cloned(a)?;
    if matches!(recv.value, RValue::Instance(_)) {
...
pub(crate) fn op_class(vm: &mut VM, operand: &Fetched) -> Result<(), Error> {
    ...
    // `class Foo` later is not a new subclass. The namespace's own constant
    // table is what answers this: `vm.consts` is keyed by bare name whatever
    // the namespace, so a nested `P::Foo` would make a later top-level `Foo`
    // look like a reopen and swallow its hook.
    let is_new = match &parent_module {
        Some(parent) => !parent.consts.borrow().contains_key(&name),
        None => !vm.object_class.consts.borrow().contains_key(&name),
    };
