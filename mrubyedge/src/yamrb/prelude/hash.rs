...
fn mrb_hash_each_half(
    ...
    let block = args
        .first()
        .filter(|block| matches!(block.value, RValue::Proc(_)))
        .ok_or_else(|| Error::ArgumentError(format!("{} expects a block", name)))?;
...
fn mrb_hash_dig(vm: &mut VM, args: &[Rc<RObject>]) -> Result<Rc<RObject>, Error> {
    // Only a trailing Proc is the implicit block. A Proc anywhere else is a key.
    let keys = match args.split_last() {
        Some((last, rest)) if matches!(last.value, RValue::Proc(_)) => rest,
        _ => args,
    };
    if keys.is_empty() {
        return Err(Error::ArgumentError(
            "Hash#dig expects at least one key".to_string(),
        ));
    }
    ...
    for (i, key) in keys.iter().enumerate() {
        let next = match &current.value {
            ...
            RValue::Nil => None,
            _ => return mrb_funcall(vm, Some(current.clone()), "dig", &keys[i..]),
        };
        match next {
