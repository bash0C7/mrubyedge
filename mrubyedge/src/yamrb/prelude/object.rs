...
pub fn mrb_object_not_eq(vm: &mut VM, args: &[Rc<RObject>]) -> Result<Rc<RObject>, Error> {
    ...
    if matches!(lhs.value, RValue::Instance(_)) {
        // A cmethod body only runs once the send machinery that called it has
        // already moved the register window past the caller's frame, so
        // mrb_funcall here can't clobber it the way calling it straight from
        // an opcode handler (e.g. op_eq) could.
