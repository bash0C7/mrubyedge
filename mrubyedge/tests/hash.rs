...
// Compiles and runs `code`, then calls `test_main` and converts the result to an i64.
...
fn hash_each_key_and_value_test() {
    ...
    ";
    assert_eq!(run_test_main_i("hash_each_half", code), 203);
}

#[test]
fn hash_each_key_rejects_a_non_block_argument_test() {
    // A non-Proc argument must raise, not reach mrb_call_block, which panics.
    let code = "
    def test_main
      h = { 'a' => 1 }
      begin
    ...
    ";
    assert_eq!(run_test_main_i("hash_each_key_non_block", code), 1);
}

#[test]
fn hash_dig_requires_at_least_one_key_test() {
    let code = "
    def test_main
      h = { 'a' => 1 }
      begin
