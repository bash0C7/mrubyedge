...
fn included_fires_and_can_extend_the_including_class() {
    ...
    A.cm
    ";
    assert_eq!(run_top_level_s("included_hook", code), "from ClassMethods");
}
...
fn what_included_puts_on_a_class_reaches_its_subclasses() {
    ...
    B.cm
    ";
    assert_eq!(run_top_level_s("included_hook_inherited", code), "ok");
}
...
fn including_a_module_without_the_hook_is_still_fine() {
    ...
    A.new.hello
    ";
    assert_eq!(run_top_level_s("included_hook_absent", code), "hi");
}
...
fn inherited_fires_with_the_new_subclass() {
    ...
    ";
    assert_eq!(run_top_level_s("inherited_hook", code), "One,Two");
}
...
fn the_class_body_still_runs_after_the_hook() {
    ...
    Sub.new.value
    ";
    assert_eq!(run_top_level_s("inherited_hook_body", code), "body ran");
}
...
fn reopening_a_class_does_not_fire_inherited_again() {
    ...
    Base.count.to_s
    ";
    assert_eq!(run_top_level_s("inherited_hook_reopen", code), "1");
}

#[test]
fn a_namespaced_class_does_not_make_a_later_top_level_one_look_reopened() {
    // `P::Dup` and `Dup` are two different classes, so both are new
    // subclasses and both fire the hook.
    let code = "
    class Base
      def self.log
        @log ||= []
    ...
    ";
    assert_eq!(
        run_top_level_s("inherited_hook_namespaced", code),
        "P::Dup,Dup"
    );
}
