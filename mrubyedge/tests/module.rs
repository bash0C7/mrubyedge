...
#[test]
fn a_module_bodys_ivar_is_visible_from_a_reader_method() {
...
fn module_class_eval_defines_an_instance_method() {
    ...
    ";
    assert_eq!(run_test_main_i("module_class_eval", code), 99);
}
...
fn module_private_instance_methods_is_always_empty() {
    ...
    ";
    assert_eq!(run_test_main_i("module_private_instance_methods", code), 0);
}
...
fn module_visibility_declarations_are_accepted_as_noops() {
    ...
    ";
    assert_eq!(
        run_test_main_s("module_visibility_noop", code),
        "2,nil,:pub_method"
    );
}
