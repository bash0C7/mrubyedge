...
// Uses inspect, not to_s: Class#to_s does not exist in v2.0.0, and adding it
// is a different unit's change.
#[test]
fn basic_object_is_the_root_of_the_hierarchy_test() {
    ...
    def test_main
      Bare.ancestors.map { |k| k.inspect }.join(',')
    end
    ";
    assert_eq!(
        run_test_main_s("basic_object_ancestors", code),
        "Bare,BasicObject"
    );
}
...
fn a_basic_object_subclass_has_no_object_methods_test() {
    ...
    ";
    assert_eq!(run_test_main_s("basic_object_no_to_s", code), "no to_s");
}
...
fn different_objects_are_not_identical_test() {
    ...
    ";
    assert_eq!(run_test_main_s("equal_identity_diff", code), "false");
}
...
fn an_object_is_identical_to_itself_test() {
    ...
    ";
    assert!(run_test_main_b("equal_identity_self", code));
}
...
fn string_literals_are_not_identical_test() {
    ...
    ";
    assert_eq!(
        run_test_main_s("equal_identity_string_literals", code),
        "false"
    );
}
...
fn bang_negates_false_test() {
    ...
    ";
    assert!(run_test_main_b("bang_false", code));
}
...
fn bang_negates_nil_test() {
    ...
    ";
    assert!(run_test_main_b("bang_nil", code));
}
...
fn bang_of_a_truthy_string_is_false_test() {
    ...
    ";
    assert_eq!(run_test_main_s("bang_string", code), "false");
}
...
fn bang_of_zero_is_false_test() {
    ...
    ";
    assert_eq!(run_test_main_s("bang_zero", code), "false");
}
...
fn case_when_a_class_asks_whether_the_value_is_one_test() {
    ...
    ";
    assert_eq!(
        run_test_main_s("case_when_class", code),
...
fn case_when_a_class_follows_the_ancestors_test() {
    ...
    ";
    assert_eq!(
        run_test_main_s("case_when_ancestors", code),
        "animal,loud,other"
    );
}
